...
class Scraper(object):
    ...
    def getSummary(self, title, isUrl = False):
        result = []
        result.append(self.getTitle(title) if isUrl else title)
        result.append("URL:" + title if isUrl else "URL:" + self.url_map[title])
        timeframe = self.getTimeFrame(title)
        result.append("Start Date: " + timeframe[0] + "     End Date: " + timeframe[1])
        result.append(self.getPurpose(title))
        result.append(self.getCriteria(title))
        # result.append(self.getExclusion(title))
        return result

    def getPreview(self,title, isUrl = False):
        result = self.getTitle(title) if isUrl else title + "\n"
        timeframe = self.getTimeFrame(link = title)
        result += "Start Date: " + timeframe[0] + "     "
        result += "End Date: " + timeframe[1] + "\n"
        trialPurpose = self.getPurpose(link = title)
    ...
    def getTitle(url):
        ...
        for tag in soup.find_all("title"):
            line = Scraper.removeMarkup(str(tag))
            end = line.find(" - F")
            if(end != -1):
                line = line[: end]
                return line + "\n"
